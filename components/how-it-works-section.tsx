...
import { Eye, Cpu, RefreshCw, Sun, Battery, Fan, ArrowRight, ArrowDown } from "lucide-react";
        ...
        <div className="grid md:grid-cols-3 gap-x-16 gap-y-16">
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                {/* THAY ĐỔI: Lớp phủ phát sáng màu Neon phía sau khi hover */}
                ...
                {/* Icon - THAY ĐỔI: Thêm hiệu ứng shadow phát sáng nhẹ và đổi màu nền */}
                <div className="relative z-10 w-12 h-12 rounded-xl flex items-center justify-center bg-cyan-500/10 border border-cyan-400/15 mb-4 transition-all duration-300 group-hover:bg-cyan-400 group-hover:text-slate-950 group-hover:shadow-[0_0_15px_rgba(34,211,238,0.5)]">
                  {/* THAY ĐỔI: Icon sẽ scale và xoay nhẹ (rotate) tạo cảm giác cơ khí/công nghệ */}
                ...
                {/* Title & Desc - Giữ nguyên logic chỉ thêm mượt chuyển động */}
                ...
                {/* Arrows Logic (Dựa trên index hiển thị) - Giữ nguyên */}
                {index === 0 && <ArrowRight className="hidden md:block absolute top-1/2 -right-12 -translate-y-1/2 w-6 h-6 text-cyan-500" />}
                {index === 1 && <ArrowRight className="hidden md:block absolute top-1/2 -right-12 -translate-y-1/2 w-6 h-6 text-cyan-500" />}
                {index === 2 && <ArrowDown className="hidden md:block absolute left-1/2 -bottom-10 -translate-x-1/2 w-6 h-6 text-cyan-500" />}
                {index === 3 && <ArrowRight className="hidden md:block absolute top-1/2 -right-12 -translate-y-1/2 rotate-180 w-6 h-6 text-cyan-500" />}
                {index === 4 && <ArrowRight className="hidden md:block absolute top-1/2 -right-12 -translate-y-1/2 rotate-180 w-6 h-6 text-cyan-500" />}
              </motion.div>
            );
          })}
        </div>
